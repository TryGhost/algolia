...
After preflight succeeds, the CLI runs its existing index-settings setup. It then uses the legacy slug filter to delete all indexed records matching each unique fetched, non-ignored slug before saving the complete new record set. This replacement step removes stale heading or continuation records when content shrinks and makes repeated runs converge on the same records. Posts excluded by `--skip` are not fetched; posts excluded by `ignore_slugs` are fetched but are neither deleted nor saved.
