...
if (!replayOrigin || !contentApiKey) {
    throw new Error('Network denial probe requires its replay environment.'); // eslint-disable-line ghost/ghost-custom/no-native-error
}
...
const readWithHttp = () => new Promise((resolve, reject) => {
...
const readWithNet = () => new Promise((resolve, reject) => {
    ...
    socket.on('end', () => {
        ...
        while (cursor < chunkedBody.length) {
            ...
            const size = Number.parseInt(chunkedBody.subarray(cursor, lineEnd).toString('ascii'), 16);
            if (size === 0) {
                break;
            }
            cursor = lineEnd + 2;
...
const expectDenied = async (name, start) => {
    try {
        ...
        if (!/denied (?:non-loopback (?:network request|socket)|Unix socket)/u.test(error.message)) {
            throw error;
        }
        return name;
    }

    throw new Error(`${name} unexpectedly allowed an external request.`); // eslint-disable-line ghost/ghost-custom/no-native-error
};
...
const main = async () => {
    ...
    const denied = await Promise.all([
        ...
        expectDenied('http.request options', () => http.request({protocol: 'http:', hostname: 'example.invalid', path: '/'})),
        expectDenied('http.get hostname override', () => http.get(replayOrigin, {hostname: 'example.invalid'})),
        expectDenied('http.get port override', () => http.get(replayOrigin, {port: 80})),
        expectDenied('http.get protocol override', () => http.get(replayOrigin, {protocol: 'https:'})),
        ...
        expectDenied('https.request options', () => https.request({protocol: 'https:', hostname: 'example.invalid', path: '/'})),
        ...
        expectDenied('net.createConnection', () => net.createConnection({port: 443, host: 'example.invalid'})),
        expectDenied('net.Socket.connect', () => new net.Socket().connect({host: 'example.invalid', port: 443})),
        expectDenied('net.Socket.connect path', () => new net.Socket().connect({path: '/tmp/algolia-network-denial-probe.sock'})),
        expectDenied('tls.connect', () => tls.connect(443, 'example.invalid')),
        expectDenied('tls.TLSSocket.connect', () => new tls.TLSSocket(new net.Socket()).connect({host: 'example.invalid', port: 443}))
    ]);
