...
if (!process.send || !fixtureDirectory || !requestLogPath || !contentApiKey || !replayMode) {
    throw new Error('Ghost replay server requires IPC and its fixture environment.'); // eslint-disable-line ghost/ghost-custom/no-native-error
}
...
const firstPost = Buffer.from(JSON.stringify({
...
const replayPlans = {
    automatic: [
        {query: {key: contentApiKey, include: 'tags,authors', limit: '100', page: '1'}, body: page1},
        {query: {key: contentApiKey, include: 'tags,authors', limit: '100', page: '2'}, body: page2}
    ],
    'limit-one': [
...
if (!replayPlan) {
    throw new Error(`Unknown Ghost replay mode: ${replayMode}`); // eslint-disable-line ghost/ghost-custom/no-native-error
}
...
const server = http.createServer((request, response) => {
    ...
    if (!valid) {
        ...
        response.end(JSON.stringify({errors: [{message: 'Unexpected Ghost Content API request.'}]}));
        return;
    }
